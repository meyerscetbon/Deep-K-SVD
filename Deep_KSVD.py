...
class mydataset_sub_images(Dataset):
    ...
    def __init__(self, root_dir, image_names, sub_image_size, sigma, transform = None):
        ...
        self.number_sub_images = int((w-sub_image_size+1)*(h-sub_image_size+1))
...
class DenoisingNet_MLP(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        fold = torch.nn.functional.fold(x_pred, output_size=(w,h),
                        kernel_size =(self.patch_size,self.patch_size))

        norm = torch.nn.functional.fold(normalize, output_size=(w,h),
                            kernel_size =(self.patch_size,self.patch_size))
...
class DenoisingNet_MLP_2(torch.nn.Module):
    def __init__(self, patch_size, D_in, H_1, H_2, H_3, D_out_lam, T, min_v, max_v, Dict_init, c_init, w_1_init, w_2_init, device):
...
class DenoisingNet_MLP_3(torch.nn.Module):
    def __init__(self, patch_size, D_in, H_1, H_2, H_3, D_out_lam, T, min_v, max_v, Dict_init, c, w_1_init, w_2_init, w_3_init, device):
